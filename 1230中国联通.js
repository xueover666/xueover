...
### 20251228
v1.23:
- 🛡️ 抢50话费安全升级：新增“精准守时”机制。脚本于 9:58 启动后，会完成预登录与活动链接提取，随后**自动挂起等待**，直到 **10:00:00** 准时唤醒执行抢购。
- 🚫 防黑号优化：彻底杜绝了在 10:00 前因过早循环请求导致的联通风控（黑号）问题，同时确保秒杀时无网络连接耗时，实现毫秒级响应。

v1.22:
- 🐛 修复阅读专区：新增 `history/add` 初始化逻辑，彻底解决模拟阅读时报错 `{"code":"9999","message":"不存在阅读记录"}` 的问题。
- 📢 增强通知推送：阅读专区与权益超市的抽奖环节，现在无论中奖、未中奖、库存拦截或次数耗尽，均会推送到通知，状态一目了然。
- 🛒 优化权益超市：重构库存监控日志格式（如：【奖品】监测到放水...），精准识别并区分“库存/风控拦截”、“纯空气”与“今日次数耗尽”三种未中奖状态。
- ⚡ 优化抢50话费：抢购循环次数调整为200次，新增全状态通知（无论是抢到、库存不足0108、还是全程限流0109，每个账号均会推送最终结果通知）。

### 20251227
v1.21:
- 🐛 修复鉴权逻辑：修正 `onLine` 接口参数构造，解决“仅Token模式”因传入空 `appId` 导致鉴权失败(Code:9999)的问题。
- 🛡️ 全模式兼容：完美适配“仅Token”、“Token#AppId”、“账号密码”三种登录方式，自动判断是否携带 AppId 参数。

### 20251226
v1.20:
- 🔧 抢购策略调整：移除 18:00 场次，仅保留每日 10:00 抢购50话费券，集中资源命中早高峰。
- 🔐 鉴权增强：Token模式全面支持 `token_online#appid` 格式 (如: a3e4...#your_appid)，适配联通新版 AppId 校验逻辑，解决部分账号因缺失参数导致的登录失败问题。
...
- ⚡ 抢购模式保持：极速抢购模式（10点）继续维持“统一预登录+并发”逻辑，最大化利用IP有效期争取毫秒级响应。
...
赋值方式有三种:
1. 填账号密码 (自动获取Token - 推荐):
   export chinaUnicomCookie="18600000000#123456"
   (多账号用 & 或 换行 隔开)
2. 填Token#AppId (新鉴权模式 - 推荐):
   export chinaUnicomCookie="a3e4c1ff2xxxxxxxxx#your_appid_string"
3. 仅填Token (旧模式):
    ...
    try {
        ...
        if (result?.code === 200 && Array.isArray(result.data)) {
            ...
            if (livePrizes.length > 0) {
                // 不再单独打印“监测到权益放水”，直接在下面遍历打印，更紧凑
                livePrizes.forEach(item => {
                    const name = item.name.trim();
                    const daily = item.dailyPrizeLimit;
                    const total = item.quantity; // 只能获取当前库存
...
// 4. 任务主入口 (修改版：100次循环 + 0108不停止 + 结果全通知 + 准点爆发)
  async grab50_task() {
    // 时间判断
    const now = new Date();
    ...
    // 早场 (9:55 - 10:10)
    // 稍微放宽一点开始时间，防止脚本启动慢错过
    const isMorning = (hour === 9 && minutes >= 55) || (hour === 10 && minutes <= 10);

    if (!isMorning) {
        this.log("抢50话费: 当前不在抢购时间段(10点)，跳过");
        return;
    }
    ...
    // 1. 先获取活动链接 (这个可以提前做，不影响风控，反而节省正式抢购时的耗时)
    const taskUrl = await this.grab50_getTaskUrl();
    if (!taskUrl) {
        this.log("抢50话费: 未获取到活动URL，可能活动未开始或已结束");
        return;
    }
    this.log(`抢50话费: 获取链接成功，准备就绪...`);

    // ==========================================
    // 🕒 精准等待至 10:00:00
    // ==========================================
    const checkTime = new Date();
    // 如果当前是 9 点 (比如 9:58, 9:59)，计算距离 10:00:00 的毫秒数
    if (checkTime.getHours() === 9) {
        const targetTime = new Date();
        targetTime.setHours(10, 0, 0, 0); // 设定目标为今天的 10:00:00.000
        
        const waitMs = targetTime.getTime() - checkTime.getTime();
        
        if (waitMs > 0) {
            // 打印等待日志，显示倒计时
            this.log(`抢50话费: ⏳ 尚未到达10点，正在待机等待 ${(waitMs/1000).toFixed(2)}秒...`);
            this.log(`抢50话费: 🛡️ 此等待是为了防止提前请求导致黑号`);
            
            // 核心挂起：这里会一直等到 10:00:00 才会继续往下执行
            await appName.wait(waitMs); 
        }
    }
    // ==========================================

    this.log(`抢50话费: 🚀 时间已到 (10:00:00)，全速启动抢购！`);
    ...
    // 【修改点1】循环次数改为 100 次
    const max_loops = 100; 
    let finalResult = "UNKNOWN"; // 用于记录最终状态
    ...
    while (!stopped && count < max_loops) {
        ...
        if (res === "NO_STOCK") {
            // 【修改点2】库存不足(0108)，只记录状态，不停止 (stopped = true 已移除)
            this.log(`抢50话费: ❌ 第${count}次: 库存不足(0108) - 继续尝试`);
            finalResult = "NO_STOCK";
            // stopped = true; // <--- 这里被注释掉了，不会提前结束
            await appName.wait(100); // 稍微加一点点延迟，防止请求过密被秒封
        } else if (res === "LIMITING") {
            // 限流(0109)，记录状态，继续重试直到次数上限
            this.log(`抢50话费: ⚠️ 第${count}次: 接口限流(0109)`);
            finalResult = "LIMITING"; 
            await appName.wait(300); // 限流时稍微增加一点间隔
        } else if (typeof res === 'object' && res.status === "SUCCESS") {
            // 抢购成功
            const winInfo = await this.grab50_queryWin(res.uuid);
            const prizeName = winInfo?.data?.prizeName || "未知奖品";
            // 【通知】成功情况
            this.log(`抢50话费: 🎉 抢购成功! (耗时:${count}次) - ${prizeName}`, { notify: true });
            finalResult = "SUCCESS";
            stopped = true; // 抢到了肯定要停，不然也是浪费
        } else if (typeof res === 'object' && res.status === "ERROR") {
             // 其他错误
             this.log(`抢50话费: ❌ 第${count}次: 错误 ${res.msg}`);
             if (count === max_loops && finalResult === "UNKNOWN") finalResult = "ERROR";
             await appName.wait(200);
        }
    }

    // 【核心修改】循环结束后，如果不是成功状态，统一根据最后的状态推送一条通知
    if (finalResult === "NO_STOCK" && !stopped) {
        // 跑满了100次还是没抢到
        this.log(`抢50话费: 😭 抢购失败 (尝试${count}次均提示库存不足)`, { notify: true });
    } else if (finalResult === "LIMITING" && !stopped) {
        // 跑满了100次，且最后状态是限流
        this.log(`抢50话费: 🏁 抢购结束 (全程限流/未抢到, 共${count}次)`, { notify: true });
    } else if (finalResult !== "SUCCESS" && count >= max_loops) {
        // 其他情况跑满100次
        this.log(`抢50话费: 🏁 抢购结束 (尝试${count}次未果)`, { notify: true });
    }
...
!(async () => {
  ...
  if (isMorningRush) {
    ...
    // ==========================================
    appName.log(`\n🚨🚨🚨 [极速并发抢购模式] 🚨🚨🚨`);
    appName.log(`检测到临近抢购时间 (10点)，启动并发抢券！`);
